...
local ICON_DIR = ConfigWidgetDir .. '/brightness/'
...
local function worker(user_args)
  ...
  if type == 'icon_and_text' then
    brightness_widget.widget = Wibox.widget({
      ...
      set_value = function(self, level)
        ...
        if percentage then display_level = display_level .. '%' end
  ...
  elseif type == 'arc' then
    brightness_widget.widget = Wibox.widget({
      ...
      forced_width = size,
      paddings = 2,
      colors = { Beautiful.fg_focus, Beautiful.fg_focus, Beautiful.fg_focus },
  ...
  function brightness_widget:set(value)
    ...
    spawn.easy_async(string.format(set_brightness_cmd, value), function()
      spawn.easy_async_with_shell(get_brightness_cmd, function(out) update_widget(brightness_widget.widget, out) end)
    end)
  end
